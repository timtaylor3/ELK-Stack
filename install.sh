...
# All of the config files should
...
# yum -y localinstall elasticsearch-5.4.2.rpm
...
# yum -y localinstall kibana-5.4.2-x86_64.rpm 
...
# yum -y localinstall logstash-5.4.2.rpm
...
mkdir -p /usr/local/elk/docs
mkdir -p /usr/local/elk/grok-patterns
mkdir -p /usr/local/elk/lib
mkdir -p /usr/local/elk/supporting-scripts

# ELK configs
cp master-configs/configfiles/* /usr/local/elk/configfiles/.
...
cp -r master-configs/lib/* /usr/local/elk/lib/.
cp -r master-configs/supporting-scripts/* /usr/local/elk/supporting-scripts/.
...
# yum -y localinstall filebeat-5.4.2-x86_64.rpm
...
# Extract test data
# unzip master-configs/sample_logs/logstash.zip -d /
